...
const getData = async (id) => {
  ...
  for (const row of data) {
    ...
    const query = `SELECT column_name FROM information_schema.columns WHERE table_name = 'master_benchmarks_price' AND column_name LIKE '%${stock_no}'`;
...
const InsertdldataEureka = async (id) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // resolve(cov(data.percentage_change_array));
